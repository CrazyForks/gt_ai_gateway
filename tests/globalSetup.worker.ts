...
function clearD1Tables(): void {
    ...
    try {
        ...
        if (match) {
            ...
            const tables = Array.isArray(parsed) && parsed.length > 0 && Array.isArray(parsed[0]?.results)
...
function startTestServer(): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        const command = ['wrangler', 'dev', '--local', '--port', WORKER_PORT.toString()]
        const env: NodeJS.ProcessEnv = { ...process.env }
        env.PORT = WORKER_PORT.toString()

        console.log(`Starting test server in worker mode on port ${WORKER_PORT}`)
        ...
        testServerProcess.stdout?.on('data', (data) => {
            ...
            if (!serverStarted) {
                ...
                if (output.includes('Ready') || output.includes('localhost:' + WORKER_PORT)) {
                    serverStarted = true
                    resolve()
                }
            }
        ...
        setTimeout(() => {
            if (!serverStarted) {
                reject(new Error(`Server startup timeout (${WORKER_STARTUP_TIMEOUT}ms)`))
            }
