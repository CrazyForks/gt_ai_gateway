 ...
 */
function handleAnthropicMessages(req: IncomingMessage, res: ServerResponse): void {
  let body = ''
 ...
 */
function handleAnthropicNonStreamResponse(res: ServerResponse, data: any): void {
  const response = {
    id: `msg_${Date.now()}`,
    type: 'message',
    role: 'assistant',
...
function handleAnthropicStreamResponse(res: ServerResponse, data: any): void {
  ...
  const chunks = [
    ...
    { type: 'content_block_delta', delta: { text: ' How can I help you?' } },
  ]
