...
describe('Integration Tests', () => {
  ...
  describe('Complete User -> Vendor -> Model -> Request Workflow', () => {
    ...
    it('should complete full workflow: create vendor, model, user, and make request', async () => {
      ...
      const vendorResponse = await requestHelper.post('/vendor/create.json', vendorFixtures.VENDOR_FIXTURES.openai())
      ...
      const modelResponse = await requestHelper.post('/model/create.json', modelFixtures.createRandomModel(vendorId, modelNameValue))
      ...
      const chatRequest = mockHelper.generateOpenAIChatRequest({ model: modelName, stream: false })
      const chatResponse = await requestHelper.post('/v1/chat/completions', chatRequest, userToken)
    ...
    it('should verify cross-data associations in record', async () => {
      const recordsResponse = await requestHelper.get('/record/latest.json?limit=10')
      ...
      const vendorResponse = await requestHelper.get(`/vendor/${vendorId}`)
      expect(vendorResponse.status).toBe(200)
    ...
    it('should handle multiple requests from same user to same model', async () => {
      ...
      const response1 = await requestHelper.post('/v1/chat/completions', chatRequest, userToken)
      const response2 = await requestHelper.post('/v1/chat/completions', chatRequest, userToken)
      const response3 = await requestHelper.post('/v1/chat/completions', chatRequest, userToken)
      ...
      const recordsResponse = await requestHelper.get('/record/list.json')
  ...
  describe('Multi-User, Multi-Model Workflow', () => {
    ...
    it('should create multiple users and models', async () => {
      ...
      // Create 3 models
      for (let i = 0; i < 3; i++) {
        const modelResponse = await requestHelper.post('/model/create.json', modelFixtures.createRandomModel(vendorId, `multi-model-${i}`))
        expect(modelResponse.status).toBe(200)
        models.push({
    ...
    it('should verify each user can access each model', async () => {
      for (const user of users) {
        for (const model of models) {
          const chatRequest = mockHelper.generateOpenAIChatRequest({ model: model.name, stream: false })
          const response = await requestHelper.post('/v1/chat/completions', chatRequest, user.token)
